...
result = pd.DataFrame()
tradeable = mongod.conn.get_data("select * from all_symbol.cantrade_TOS").Ticker.tolist()
...
if continue_point >0:
    my_csv=write_my_csv("pair_ETF.csv",header,False)
else:
    my_csv=write_my_csv("pair_ETF.csv",header)
for i in tradeable:
#     start_point+=1
     for j in tradeable:
