...
class Robinhood:
    ...
    def get_historical_quotes(self, stock, interval, span, bounds=Bounds.REGULAR):
        ...
        historicals = endpoints.historicals() + "/?symbols=" + ','.join(stock).upper() + "&interval=" + interval + "&span=" + span + "&bounds=" + "regular"
    ...
    def buying_power(self):
        ...
        return float(self.portfolios()['withdrawable_amount'])
    ...
    def place_option_order(self,legs,
                    quantity,
                    price,
                    direction, 
...
#         return payload
        ...
        res = self.session.post( endpoints.option_order(),data=json.dumps(payload), timeout=15)
#         res.raise_for_status()
