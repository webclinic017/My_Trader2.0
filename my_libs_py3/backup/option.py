...
class option_trading():
    ...
    def __init__(self,robinhood, symbol,pc, buy_sell, exp = None, strike = None ):
        ...
        if exp == None:
            ...
            print (self.exp)
            self.exp= self.exp[input("Which exp date")]
            print (self.exp)
        elif type(exp) == int:
        ...
        if buy_sell == 'sell':
            ...
            self.adjusted_mark_price -=  float(self.__fetch_option_info(self.get_p_oc().iloc[0]["id"])["adjusted_mark_price"])
        elif buy_sell == 'buy':
            self.adjusted_mark_price +=  float(self.__fetch_option_info(self.get_p_oc().iloc[0]["id"])["adjusted_mark_price"])
...
#         return pd.Series(list(set(self.__fetch_option_chain().expiration_date))).sort_values().to_list()
        return pd.Series(self.__fetch_option_exp_date()).sort_values().to_list()
    
    def get_strike(self):

        print ("Ascending")
        return list(pd.DataFrame(self.trader.my_trader.get_options(self.symbol,self.exp,self.pc)).strike_price.astype(float).sort_values())
    
    
    def get_option(self):
     
        result =  pd.DataFrame(self.trader.my_trader.get_options(self.symbol,self.exp, self.pc))
    ...
    def get_option_price(self): 
        return pd.Series(self.__fetch_option_info(self.get_p_oc().iloc[0]["id"]))

    

    def current_Option(self): 
        print ("current option:\n Symbol:%s strike:%s, expiration:%s, type:%s"%(self.symbol,self.strike,self.exp,self.pc))
        get = pd.Series(self.__fetch_option_info(self.get_p_oc().iloc[0]["id"]))
