...
class future ():
    ...
    def get_future_expiration(self,year,month):
        ...
        try:
            page = r.get("https://www.cboe.com/delayedquote/futures-quotes")
...
#         return day_roll.values[0]
    ...
    def trade_action(self):
        ...
        #vix_quote = -0.0050
        short_size = int(self.initial/da.get_quote_yahoo(self.short_symbol).regularMarketPrice.values[0])
        long_size =  int(self.initial/da.get_quote_yahoo(self.long_symbol).regularMarketPrice.values[0])
        
        if vix_quote > enter_signal:
            if self.robinhood.place_buy_bulk_checkup(ticker_list=[self.short_symbol],quantity_list=[short_size],price_list=[da.get_quote_yahoo(self.short_symbol).regularMarketPrice.values[0]] )== "Trade Success!":
                log_trade(self.short_symbol,short_size, da.get_quote_yahoo(self.short_symbol).regularMarketPrice.values[0], "vix_dayroll")
                send_email(body_html="",body_content="", title = "VIX Dayroll enter Signal")
        
        elif vix_quote < exit_signal and log["size"].sum() != 0:
            if self.robinhood.place_sell_bulk_checkup(ticker_list=[self.short_symbol],quantity_list=[log["size"].sum()],price_list=[da.get_quote_yahoo(self.short_symbol).regularMarketPrice.values[0]] )== "Trade Success!":
                log_trade(self.short_symbol,-log["size"].sum(), da.get_quote_yahoo(self.short_symbol).regularMarketPrice.values[0], "vix_dayroll")
                send_email(body_html="",body_content="", title = "VIX Dayroll exit Signal")
        else:
            print ("no short trade signal")
        
        if vix_quote < buy_enter_signal:
            if self.robinhood.place_buy_bulk_checkup(ticker_list=[self.long_symbol],quantity_list=[long_size],price_list=[da.get_quote_yahoo(self.long_symbol).regularMarketPrice.values[0]])== "Trade Success!":
                log_trade(self.long_symbol,long_size, da.get_quote_yahoo(self.long_symbol).regularMarketPrice.values[0], "vix_dayroll")
            ...
            if self.robinhood.place_sell_bulk_checkup(ticker_list=[self.long_symbol],quantity_list=[log_buy["size"].sum()],price_list=[da.get_quote_yahoo(self.long_symbol).regularMarketPrice.values[0]] )== "Trade Success!":
                log_trade(self.long_symbol,-log_buy["size"].sum(), da.get_quote_yahoo(self.long_symbol).regularMarketPrice.values[0], "vix_dayroll")
