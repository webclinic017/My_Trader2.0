...
root_directory = "/home/ken/notebook/My_Trader2.0/"
...
def get_price_data(tic_list,method,interval = "30min",robinhood= None,start_date= None, end_date= None, back_day=90, last_record= False):
    ...
    if method not in ["minute","day", "self_minute","realtimeday","intraday"]:
...
class finviz:
    ...
    def all_in_one(self, symbol):
        ...
        def conver_cap (mkt_cap):
            if type(mkt_cap) != str or mkt_cap =="-":
                mkt_cap = np.NaN
            else:
                if mkt_cap[-1]=="B":
...
#     '''
gateway = "/home/ken/notebook/My_Trader2.0/record-Copy1.txt"
...
#         tradeable.loc[:,"Ticker"] = tradeable["symbol"]
        ...
        if type(my_list) ==bool:
            if test_mode:
                universe = mongod.conn.get_data("SELECT ticker,price,volume FROM all_symbol.`Screener_TOS`")[1:10]
            else:
                universe = mongod.conn.get_data("SELECT ticker,price,volume FROM all_symbol.`Screener_TOS`")
        else:
            universe = my_list
        stock = list()
        ...
        mongod.conn.to_sql(tradeable,"all_symbol","cantrade",if_exists="append")
