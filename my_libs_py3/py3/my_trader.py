...
def fix_unsettled_trade_update(ticker, size=0, partial=False):
    ...
    if not partial:
        mongod.conn.conn.cursor().execute('''
        
        UPDATE trade_log.`%s` SET size = 0
        where TimeStamp = '%s'
...
def log_trade(ticker,size, price, strategy , hold_days = None, database = "trade_log"):
    ...
    mongod.conn.to_sql(upload_data,database,ticker,if_exists = "append")
...
def get_trade_log(ticker,database = "trade_log"): 
    mongod = mongo(database)
    try:
        result = mongod.conn.get_data("select * from %s.%s order by TimeStamp"%(database,ticker))   
...
def hedge(ticker,robinhood,method = "self_minute",hedge_int = "VIXY",target_beta =0.0):
    ...
    position_value = robinhood.get_ask_price(ticker)
...
#         return self.universe['Ticker']
    ...
    def get_my_position_beta_minute(self,sv=0.05):
        ...
        for i in data.index:
            ...
            pre_price = float(self.my_trader.quote_data(i)['previous_close'])
...
#         data.Last_price = data.Last_price.astype(float)
    ...
    def get_average_cost(self,symbol):
        ...
        tmp = [x for x in pos_list if x["instrument"] == self.get_instrument(symbol)["url"].values[0]]
        if len(tmp) == 0:
            try:
